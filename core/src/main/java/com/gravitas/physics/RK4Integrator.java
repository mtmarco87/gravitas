 ...
 * bodies.
 *
 * State vector per object: [x, y, z, vx, vy, vz]
...
public class RK4Integrator {
    ...
    public void step(SimObject[] objects, int count, double dt) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            vzt[i] = vz0[i] + k3az[i] * dt;
        }
        double[] k4vx = new double[count];
        double[] k4vy = new double[count];
        double[] k4vz = new double[count];
        double[] k4ax = new double[count];
        double[] k4ay = new double[count];
        double[] k4az = new double[count];
        computeDerivatives(objects, count, xt, yt, zt, vxt, vyt, vzt, k4vx, k4vy, k4vz, k4ax, k4ay, k4az);
    ...
    private void computeDerivatives(
        ...
        for (int i = 0; i < count; i++) {
            ...
            for (int j = i + 1; j < count; j++) {
                ...
                double dy = y[j] - y[i];
                double dz_ = z[j] - z[i];
                double distSq = dx * dx + dy * dy + dz_ * dz_ + EPSILON * EPSILON;
